...
async def urgent_check():
    ...
    from app.core.database import AsyncSessionLocal
    async with AsyncSessionLocal() as db:
        await _update_subscriptions(db, type_filter="줍줍")


async def _update_subscriptions(db, type_filter: str = None):
    try:
        from app.services.subscription_service import fetch_subscriptions_from_api
        from app.models.models import Subscription

        items = await fetch_subscriptions_from_api()
        if type_filter:
        ...
        await db.commit()
        logger.info(f"청약 {len(items)}건 확인 (신규 {len(new_items)}건 저장)")
...
async def _update_transactions(db):
    try:
        from app.services.transaction_service import fetch_transactions_from_api, _get_month_range
        from app.models.models import Transaction

        today = date.today()
        yearmonth = f"{today.year}{today.month:02d}"
        start_date, end_date = _get_month_range(yearmonth)

        # 해당 월 기존 데이터 삭제 후 재적재 (중복 방지)
        await db.execute(
            delete(Transaction).where(
                Transaction.deal_date >= start_date,
                Transaction.deal_date <= end_date,
            )
        )

        items = await fetch_transactions_from_api(yearmonth)
        for item in items:
            db.add(Transaction(**item))
        await db.commit()
        logger.info(f"실거래가 {len(items)}건 저장 완료 ({yearmonth})")
    except Exception as e:
        await db.rollback()
...
async def _update_market_stats(db):
    try:
        from app.services.market_stats_service import _aggregate_from_db
        from app.models.models import MarketStat

        items = await _aggregate_from_db()
        for item in items:
            existing = await db.execute(
